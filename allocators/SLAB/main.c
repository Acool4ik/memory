...
#define PAGE_SIZE_4KB 1024 * 4
#define SLAB_SIZE_4MB 1024 * PAGE_SIZE_4KB
/**
...
void *alloc_slab(int order) {
    if(order < 0 || order > 10) return NULL;

    unsigned bytes = PAGE_SIZE_4KB * (1 << order);
    void *slab_ptr = malloc(2 * bytes);

    /* выравниваем адрес по 4MB*/
    slab_ptr = (char *)slab_ptr + bytes - ((unsigned)slab_ptr) % bytes;

    /* проверка выравнивания */
    if((unsigned)slab_ptr != ((unsigned)slab_ptr & (~(1 << order - 1))))
        return NULL;
...
int main() {
    ...
    cache_setup(&cache_item, PAGE_SIZE_4KB);

    /* аллокация */
    void *array_blocks[1024];
    for(int i = 0; i < 1024; i++)
        array_blocks[i] = cache_alloc(&cache_item);

    /* проверка */
    for(int i = 0; i < 1024; i++)
        *(long long int*)array_blocks[i] = i * 100;

    /* освобождение всей памяти */
    cache_release(&cache_item);
    return 0;
}
